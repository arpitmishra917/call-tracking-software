...
## Purpose
Blocklist checks for inbound calls.

## Responsibilities
Manage blocked callers logic for CallFlow.
...
## Backend
Implemented in `apps/api/src/routing/blocked-callers.controller.ts`.
...
## Tests
- `apps/api/test/`
- Associated `.spec.ts` files in `apps/api/src/routing/blocked-callers.controller.ts`
